...
for single_tr in bsObj.find("table", {"title":"牌告匯率"}).find("tbody").findAll("tr"):      
    ...
    currency_buy = cell[1].contents[0]
    print(currency_name, currency_rate, currency_buy)
    file_name = currency_name + '.csv'  #進行儲存檔案.csv
    now_time = strftime("%Y-%m-%d %H:%M:%S" , localtime())  #目前時間
    if not exists(file_name):
        data = [['          時間        ''|  現金賣出'  '|現金買入'] , ["%-22s|  %-7s|%-7s"%(now_time,currency_rate,currency_buy)]] #如果檔案不存在,執行這行
    else:
        data = [["%-22s|  %-7s|%-7s"%(now_time,currency_rate,currency_buy)]] #如果檔案存在,執行這行
